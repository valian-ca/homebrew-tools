...
CI results or merge authorization. Campaign state cannot be set arbitrarily.`,
  ...
  atelierd next contribution open --campaign <id> --session session-1 --token <token> --operation child-1 --ticket EXAMPLE-2`,
	...
	repair.AddCommand(nextMutation("prepare", "repair-prepare", "Prepare a tested QA/CI repair or a pre-trial environment repair (see --from schema)"), nextMutation("finish", "repair-finish", "Prove and integrate the prepared repair commit"), nextMutation("cancel", "repair-cancel", "Cancel an uncommitted repair intent"))
